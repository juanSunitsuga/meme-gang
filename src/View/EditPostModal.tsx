...
const EditPostModal: React.FC = () => {
  const { isEditPostModalOpen, closeEditPostModal, editPostData } = useModal();
  
  const [title, setTitle] = useState(editPostData?.titleEdit || '');
  const [image, setImage] = useState<File | null>(null);
  const [imagePreview, setImagePreview] = useState<string | null>(editPostData?.imageUrlEdit || null);
  const [tagInput, setTagInput] = useState<string>('');
  const [tags, setTags] = useState<string[]>(editPostData?.tagsEdit || []);
  ...
  useEffect(() => {
    if (editPostData) {
      setTitle(editPostData.titleEdit);
      setImagePreview(editPostData.imageUrlEdit);
      setTags(editPostData.tagsEdit);
    }
  }, [editPostData]);
